"use client"

import { fundValuationOverTime } from "@/lib/mock-data"
import {
  BarChart,
  Bar,
  XAxis,
  YAxis,
  ResponsiveContainer,
  LabelList,
} from "recharts"

export function FundValuationChart() {
  const maxValue = Math.max(...fundValuationOverTime.map((d) => d.fairValue + d.realisedValue))

  return (
    <div className="overflow-hidden rounded-[10px] border border-[#E8E6E0] bg-white">
      {/* Header */}
      <div className="flex items-center justify-between border-b border-[#E8E6E0] px-4 py-3">
        <h2 className="text-[13px] font-medium text-[#2C2C2A]">Fund valuation</h2>
        <span className="text-[11px] text-[#6B6963]">Quarterly · EUR millions</span>
      </div>

      {/* Chart */}
      <div className="px-4 py-3">
        {/* Legend */}
        <div className="mb-2 flex gap-3 text-[10px] text-[#6B6963]">
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-1.5 w-1.5 rounded-sm bg-[#D4C5A9]" />
            Investment
          </span>
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-1.5 w-1.5 rounded-sm bg-[#1B4D45]" />
            Fair value
          </span>
          <span className="flex items-center gap-1.5">
            <span className="inline-block h-1.5 w-1.5 rounded-sm bg-[#9FE1CB]" />
            Realised value
          </span>
        </div>

        <div className="h-[120px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              data={fundValuationOverTime}
              margin={{ top: 20, right: 5, left: 5, bottom: 5 }}
              barGap={3}
              barCategoryGap="25%"
            >
              <XAxis
                dataKey="period"
                axisLine={false}
                tickLine={false}
                tick={{ fontSize: 9, fill: "#6B6963" }}
              />
              <YAxis hide domain={[0, maxValue * 1.15]} />
              <Bar dataKey="investment" fill="#D4C5A9" radius={[3, 3, 0, 0]}>
                <LabelList
                  dataKey="investment"
                  position="top"
                  style={{ fontSize: 8, fill: "#6B6963" }}
                />
              </Bar>
              <Bar dataKey="fairValue" fill="#1B4D45" radius={[3, 3, 0, 0]}>
                <LabelList
                  dataKey="fairValue"
                  position="top"
                  style={{ fontSize: 8, fill: "#6B6963" }}
                />
              </Bar>
              <Bar dataKey="realisedValue" fill="#9FE1CB" radius={[3, 3, 0, 0]}>
                <LabelList
                  dataKey="realisedValue"
                  position="top"
                  style={{ fontSize: 8, fill: "#6B6963" }}
                  formatter={(value: number) => (value > 0 ? value : "")}
                />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
    </div>
  )
}
